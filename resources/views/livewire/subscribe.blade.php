    ...
    <x-baseview title="{{ __('Subscriptions') }}">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
            @foreach($subscriptions as $subscription)
                <div class="overflow-hidden text-center border rounded-md shadow cursor-pointer hover:shadow-lg"
                    wire:click="subscriptionSelected('{{ $subscription->id }}')">
                    <p class="p-5 text-5xl font-bold text-white bg-primary-400">
                        <span
                            class="text-3xl font-medium">{{ setting('currency', '$') }}</span>
                        {{ $subscription->amount }}
                    </p>
                    <p class="px-2 mt-4 mb-1 text-xl">{{ $subscription->name }}</p>
                    <p class="px-2 mb-4 text-2xl font-medium">{{ $subscription->days }} <span
                            class="text-xl font-medium">{{ _('Days') }}</span></p>
                </div>
            @endforeach
        </div>
    </x-baseview>
